(function (window, document, $, undefined) {
  ...
  async function sendMessage() {
    ...
    // Display each file name in the chat, but read the content to include in the combined message
    if (files.length > 0) {
      for (const file of files) {
        ...
        // Read the file content to include in the message sent to OpenAI
        const fileContent = await readFileContent(file);
        combinedMessage += `\n\n[File: ${file.name}]\n${fileContent}`;
      }
    }
  
    // Send the combined message (text + file content) to OpenAI
    const autoReply = await getOpenAIResponse(combinedMessage);
